...
//            System.out.println(line);
    ...
    public HashMap<Integer, Double> computeFeatures(String[] terms, String extId) throws Exception {
        ...
        // f18: Average term length in the page body
        if(this.featureIdx.contains(18)){
            try{
                TermVector vec = new TermVector(docid, "body");
                int termCount = vec.positionsLength();
                int numStopwords = 0;
                double bodyLen = 0.0;
                for(int i = 0; i < termCount; i++){
                    int stemIndex = vec.stemAt(i);
                    if(stemIndex == 0){   // encounter a stop word
                        numStopwords++;
                        continue;
                    }
                    String stem = vec.stemString(stemIndex);
                    bodyLen += stem.length();
                }
                double avgTermLen = bodyLen / (termCount - numStopwords);
                features.put(18, avgTermLen * 1.0);
            } catch(Exception e){
                features.put(18, -1.0);
            } // marker of invalid score
        }
